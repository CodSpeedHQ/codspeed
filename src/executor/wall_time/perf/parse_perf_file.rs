...
#[derive(Default)]
pub struct ProcessMountedModule {
...
pub struct MemmapRecordsOutput {
    ...
    pub mounted_modules_by_path: HashMap<PathBuf, MountedModule>,
...
pub fn parse_for_memmap2<P: AsRef<Path>>(
    ...
    let mut mounted_modules_by_path = HashMap::<PathBuf, MountedModule>::new();
    ...
    while let Some(record) = record_iter.next_record(&mut perf_file).unwrap() {
        ...
        match record.record_type {
            ...
            RecordType::MMAP2 => {
                ...
                process_mmap2_record(mmap2_record, &mut mounted_modules_by_path);
            }
            _ => continue,
        }
    }
    ...
    let tracked_pids: HashSet<pid_t> = match pid_filter {
        PidFilter::All => mounted_modules_by_path
            .iter()
            .flat_map(|(_, mounted)| mounted.pids())
            .collect(),
...
fn process_mmap2_record(
    ...
    mounted_modules_by_path: &mut HashMap<PathBuf, MountedModule>,
) {
    ...
    let mounted_module = mounted_modules_by_path
        .entry(record_path.clone())
        .or_default();

    let process_mounted_module = mounted_module
        .process_mounted_module
        .entry(record.pid)
        .or_default();

    // Extract module symbols if it's no module symbol from path
    if mounted_module.module_symbols.is_none() {
        match ModuleSymbols::from_elf(&record_path) {
            Ok(symbols) => mounted_module.module_symbols = Some(symbols),
            Err(error) => {
    ...
    process_mounted_module.symbols_load_bias = Some(load_bias);
    ...
    match unwind_data_from_elf(
        ...
        Ok((unwind_data, process_unwind_data)) => {
            mounted_module.unwind_data = Some(unwind_data);
            process_mounted_module.process_unwind_data = Some(process_unwind_data);
        }
        Err(error) => {
