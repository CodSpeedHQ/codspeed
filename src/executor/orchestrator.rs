...
impl Orchestrator {
    ...
    pub async fn execute(
        ...
        if !exec_targets.is_empty() {
            // exec-harness is a hidden subcommand of this binary: re-exec ourselves.
            let exec_harness = InternalCommands::ExecHarness(ExecHarnessArgs { args: vec![] })
                .get_shell_command()?;

            let pipe_cmd =
                multi_targets::build_exec_targets_pipe_command(&exec_harness, &exec_targets)?;
