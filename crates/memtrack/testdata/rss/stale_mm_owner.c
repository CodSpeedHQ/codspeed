 ...
 *
 * A child faults a page (registering mm0's hash as owned by that pid), execs
 * into "big" (freeing mm0 while the pid lives on), and idles holding REGION_MIB
 * of anon RSS on mm1. Each subsequent churn fork then allocates an mm_struct
 * from the same slab and populates it from the PARENT's context before tearing
 * it down from the child's, so both updates are out-of-context; mm0's slot sits
 * at the head of the per-cpu freelist, so the whole burst hashes to mm0's id.
 *
 * Everything is pinned to one CPU to keep that freelist LIFO.
 */
#define _GNU_SOURCE
#include <sched.h>
#include <signal.h>
...
/* Keeps the mm_struct freed at exec at the head of this CPU's slab freelist.
 * Fatal, because failing here would silently disable the reuse path. */
...
int main(int argc, char** argv) {
    ...
    /* run_big writes the report right after its memset and the harness never
     * pre-creates that path, so its existence means the region is resident. */
